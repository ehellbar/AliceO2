...
/// \cond CLASSIMP
ClassImp(o2::TPC::AliTPCPoissonSolver)
/// \endcond

using namespace o2::TPC;


// FIXME: the following values should come from the database. todo: update values for GEM TPC and move to a location more appropriate
const Double_t o2::TPC::AliTPCPoissonSolver::fgkTPCZ0    = 249.7;     ///< nominal gating grid position
const Double_t o2::TPC::AliTPCPoissonSolver::fgkIFCRadius=  83.5;     ///< radius which renders the "18 rod manifold" best -> compare calc. of Jim Thomas
// compare gkIFCRadius=  83.05: Mean Radius of the Inner Field Cage ( 82.43 min,  83.70 max) (cm)
const Double_t o2::TPC::AliTPCPoissonSolver::fgkOFCRadius= 254.5;     ///< Mean Radius of the Outer Field Cage (252.55 min, 256.45 max) (cm)
const Double_t o2::TPC::AliTPCPoissonSolver::fgkZOffSet  =   0.2;     ///< Offset from CE: calculate all distortions closer to CE as if at this point
const Double_t o2::TPC::AliTPCPoissonSolver::fgkCathodeV = -100000.0; ///< Cathode Voltage (volts)
const Double_t o2::TPC::AliTPCPoissonSolver::fgkGG       =     -70.0; ///< Gating Grid voltage (volts)

const Double_t o2::TPC::AliTPCPoissonSolver::fgkdvdE = 0.0024; ///< [cm/V] drift velocity dependency on the E field (from Magboltz for NeCO2N2 at standard environment)

const Double_t o2::TPC::AliTPCPoissonSolver::fgkEM = -1.602176487e-19/9.10938215e-31; ///< charge/mass in [C/kg]
const Double_t o2::TPC::AliTPCPoissonSolver::fgke0 = 8.854187817e-12;                 ///< vacuum permittivity [A·s/(V·m)]

Double_t o2::TPC::AliTPCPoissonSolver::fgExactErr = 1e-4;
Double_t o2::TPC::AliTPCPoissonSolver::fgConvErr = 1e-3;
...
void AliTPCPoissonSolver::PoissonRelaxation2D(TMatrixD &arrayV, TMatrixD &chargeDensity, Int_t rows, Int_t columns, Int_t maxIterations) {
  ...
  if ( !IsPowerOfTwo(rows-1) ) {
    // AliError("PoissonRelaxation - Error in the number of rows. Must be 2**M - 1"); // removed
    LOG(ERROR) << "PoissonRelaxation - Error in the number of rows. Must be 2**M - 1" << FairLogger::endl;
    return;
  }
  
  if ( !IsPowerOfTwo(columns-1) ) {
    // AliError("PoissonRelaxation - Error in the number of columns. Must be 2**N - 1"); // removed
    LOG(ERROR) << "PoissonRelaxation - Error in the number of column. Must be 2**N - 1" << FairLogger::endl;
    return;
  } 
  ...
  for ( Int_t count = 0 ; count < loops ; count++ ) {  
    ...
    for ( Int_t i = iOne ; i < rows-1 ; i+=iOne ) {
      Float_t radius = o2::TPC::AliTPCPoissonSolver::fgkIFCRadius + i*gridSizeR ;
    ...
    for ( Int_t i = iOne ; i < rows-1 ; i += iOne ) {
      Float_t radius = o2::TPC::AliTPCPoissonSolver::fgkIFCRadius + iOne*gridSizeR ;
...
void AliTPCPoissonSolver::PoissonMultigrid2D(TMatrixD &arrayV, TMatrixD &chargeDensity, Int_t rows, Int_t columns) {
  ...
  if ( !IsPowerOfTwo(rows-1) ) {
    ...
    LOG(ERROR) << "PoissonMultigrid - Error in the number of rows. Must be 2**M - 1" << FairLogger::endl;
    return;
  }
  if ( !IsPowerOfTwo(columns-1) ) {
    // AliError("PoissonMultigrid - Error in the number of columns. Must be 2**N - 1");
    LOG(ERROR) << "PoissonMultigrid - Error in the number of columns. Must be 2**N - 1" << FairLogger::endl;
    return;
  } // removed
  ...
  Int_t loops = TMath::Max(ngridrows, ngridcols) ;      // Calculate the number of loops for the binary expansion
	...
	// AliInfo(Form("ngridrows=%d, ngridcols=%d, loops=%d, nmgcycles=%d",ngridrows,ngridcols,loops,fMgParameters.NMGCYCLE)); // removed
        LOG(INFO) << "ngridrows=" << ngridrows << ", ngridcols=" << ngridcols << ", loops=" << loops << ", nmgcycles=" << fMgParameters.NMGCYCLE << FairLogger::endl;
	...
	if (fMgParameters.cycleType == kFCycle) {
		...
		// AliInfo(Form("Do full cycle")); // removed
                LOG(INFO) << "Do full cycle" << FairLogger::endl;
 		// FMG
		...
		for ( Int_t i = 1 ; i < trows-1 ; i++ ) {
			radius = o2::TPC::AliTPCPoissonSolver::fgkIFCRadius + i*h ;
		...
		// 2. VCycle
		// AliInfo(Form("Do V cycle")); // removed
                LOG(INFO) << "Do V cycle" << FairLogger::endl;
...
void AliTPCPoissonSolver::PoissonRelaxation3D( TMatrixD**arrayofArrayV, TMatrixD**arrayofChargeDensities,
  ...
  const Float_t  ratioZ      =  gridSizeR*gridSizeR / (gridSizeZ*gridSizeZ) ;
	...
	// AliInfo(Form("in Poisson Solver 3D rows=%d, cols=%d, phislices=%d \n",rows,columns,phislices)); // removed
        LOG(INFO) << Form("in Poisson Solver 3D rows=%d, cols=%d, phislices=%d \n",rows,columns,phislices) << FairLogger::endl;
  ...
  if ( !IsPowerOfTwo((rows-1))    ) {
    // AliError("Poisson3DRelaxation - Error in the number of rows. Must be 2**M - 1"); // removed
    LOG(ERROR) << "Poisson3DRelaxation - Error in the number of rows. Must be 2**M - 1" << FairLogger::endl;
     return; }
  if ( !IsPowerOfTwo((columns-1)) ) {
    // AliError("Poisson3DRelaxation - Error in the number of columns. Must be 2**N - 1"); // removed
    LOG(ERROR) << "Poisson3DRelaxation - Error in the number of columns. Must be 2**N - 1" << FairLogger::endl;
     return; }
  if ( phislices <= 3   )  {
    // AliError("Poisson3DRelaxation - Error in the number of phislices. Must be larger than 3"); // removed
    LOG(ERROR) << "Poisson3DRelaxation - Error in the number of phislices. Must be larger than 3" << FairLogger::endl;
    return; }
  if  ( phislices > 1000 ) {
    // AliError("Poisson3D  phislices > 1000 is not allowed (nor wise) "); // removed
    LOG(ERROR) << "Poisson3D  phislices > 1000 is not allowed (nor wise) " << FairLogger::endl;
    return; }
 ...
 // }
  // end of array init for error
	...
	printf("allocating standard vector = %d\n",loops);
  ...
  // AliSysInfo::AddStamp("3DInit", 10,0,0); // removed todo
	...
	// AliSysInfo::AddStamp("3Diter", 20,count,0); // removed todo
		...
		//}
					...
					if (count == loops-1)
					{
						...
						if ((*fErrorConvergenceNormInf)(k-1)  <= fgConvErr) {
						  ...
						  LOG(INFO) << Form("Exact Err: %f, Iteration : %d", (*fError)(k - 1), k - 1) << FairLogger::endl;
...
) 
{
	...
	const Float_t  gridSizeR   =  (o2::TPC::AliTPCPoissonSolver::fgkOFCRadius-o2::TPC::AliTPCPoissonSolver::fgkIFCRadius) / (rows-1); // h_{r}
  const Float_t  gridSizePhi =  TMath::TwoPi()/phislices;  // h_{phi}
  const Float_t  gridSizeZ   =  o2::TPC::AliTPCPoissonSolver::fgkTPCZ0 / (columns-1) ; // h_{z}
  ...
  // AliInfo(Form("in Poisson Solver 3D multigrid  rows=%d, cols=%d, phislices=%d \n",rows,columns,phislices)); // removed
  LOG(INFO) << "in Poisson Solver 3D multigrid  rows=" << rows << ", cols=" << columns << ", phislices=" << phislices << FairLogger::endl;
  
  // Check that the number of rows and columns is suitable for a binary expansion
  if ( !IsPowerOfTwo((rows-1))    ) {
    // AliError("Poisson3DMultigrid - Error in the number of rows. Must be 2**M + 1"); // removed
    LOG(ERROR) << "Poisson3DMultigrid - Error in the number of rows. Must be 2**M + 1" << FairLogger::endl;
    return; }
  if ( !IsPowerOfTwo((columns-1)) ) {
    // AliError("Poisson3DMultigrid - Error in the number of columns. Must be 2**N - 1"); // removed
    LOG(ERROR) << "Poisson3DMultigrid - Error in the number of columns. Must be 2**N - 1" << FairLogger::endl;
    return; }
  if ( phislices <= 3   )  {
    // AliError("Poisson3DMultigrid - Error in the number of phislices. Must be larger than 3"); // removed
    LOG(ERROR) << "Poisson3DMultigrid - Error in the number of phislices. Must be larger than 3" << FairLogger::endl;
    return; }
  if  ( phislices > 1000 ) {
    // AliError("Poisson3D  phislices > 1000 is not allowed (nor wise) "); // removed
    LOG(ERROR) << "Poisson3D  phislices > 1000 is not allowed (nor wise) " << FairLogger::endl;
  ...
  //AliInfo(Form("Allocating memory=%d \n",loops));
	...
	// AliInfo(Form("Finish allocation memeor\n")); // removed
        LOG(INFO) << "Finish allocation memeor" << FairLogger::endl;
	...
	// AliInfo(Form("vectprs memeor: %d\n",rows)); // removed
        LOG(INFO) << "vectprs memeor: " << rows << FairLogger::endl;
...
void AliTPCPoissonSolver::PoissonMultigrid3D( TMatrixD**arrayofArrayV, TMatrixD**arrayofChargeDensities,
  ...
  // AliInfo(Form("in Poisson Solver 3D multigrid  rows=%d, cols=%d, phislices=%d \n",rows,columns,phislices)); // removed
    LOG(INFO) << Form("in Poisson Solver 3D multigrid  rows=%d, cols=%d, phislices=%d \n",rows,columns,phislices) << FairLogger::endl;
  ...
  while (nncols >>= 1) ngridcols++;
	...
	// AliInfo(Form("ngridrows=%d, ngridcols=%d, ngridphis=%d",ngridrows,ngridcols,ngridphis)); // removed
    LOG(INFO) << Form("ngridrows=%d, ngridcols=%d, ngridphis=%d",ngridrows,ngridcols,ngridphis) << FairLogger::endl;
  Int_t loops = TMath::Max(ngridrows, ngridcols) ;      // Calculate the number of loops for the binary expansion
	...
	if (fMgParameters.cycleType == kFCycle)  {
		...
		for (Int_t count = 2 ; count <= loops ; count++ ) {		
			...
			// AliInfo(Form("Restrict3D, trows=%d, tcolumns=%d, newphislices=%d, oldphislices=%d\n",trows,tcolumns,tphislices,otphislices)); // removed  
    LOG(INFO) << Form("Restrict3D, trows=%d, tcolumns=%d, newphislices=%d, oldphislices=%d\n",trows,tcolumns,tphislices,otphislices) << FairLogger::endl;
...
void AliTPCPoissonSolver::AddInterp3D(TMatrixD **curArrayV,TMatrixD **curArrayVC,const Int_t trows,const Int_t tcolumns, const Int_t newphislices, const Int_t oldphislices) {
	...
	//const Float_t  h   =  (o2::TPC::AliTPCPoissonSolver::fgkOFCRadius-o2::TPC::AliTPCPoissonSolver::fgkIFCRadius) / ((trows-1)/2); // h_{r}  
