...
SCContainer::SCContainer() :
  ...
  mPoissonSolver(),
  mSpaceCharge(65,65,72,5,3,1,3) // TODO: defaults (so far,desired) nRRows=nZColumns=(17,129), nPhiSlices=(18,144), interpolationorder=(1,5), irregulargridsize=(3,?), strategy=1(kUseInterpolator), rbfKernelType(no default)
{
...
SCContainer::SCContainer(Int_t nRRows, Int_t nZColumns, Int_t nPhiSlices, Int_t interpolationorder) :
  ...
  mLookupTables(kFALSE),
  mPoissonSolver(),
  mSpaceCharge(nRRows,nZColumns,nPhiSlices,interpolationorder,3,1,3) // TODO: default values for irregulargridsize, strategy (should be 1=kUseInterpolator), rbfKernelType
{
  mSpaceCharge.SetOmegaTauT1T2(OMEGATAU,1.,1.);
  mSpaceCharge.SetPoissonSolver(&mPoissonSolver);
}
