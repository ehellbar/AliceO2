...
class InputSpan
{
 public:
  ...
  InputSpan(std::function<size_t(size_t)> nofPartsGetter,
            ...
            std::function<fair::mq::Message*(size_t, DataRefIndices)> messageGetter,
            size_t size);
  ...
  /// Return the fair::mq::Message* for the part described by @a indices in slot @a slotIdx
  [[nodiscard]] fair::mq::Message* getMessageAtIndices(size_t slotIdx, DataRefIndices indices) const
  {
    return mMessageGetter(slotIdx, indices);
  }
  ...
  struct PartRange {
    ...
    [[nodiscard]] fair::mq::Message* getMessageAtIndices(DataRefIndices idx) const { return span->getMessageAtIndices(slot, idx); }
 ...
 private:
  ...
  std::function<fair::mq::Message*(size_t, DataRefIndices)> mMessageGetter;
  size_t mSize;
};
