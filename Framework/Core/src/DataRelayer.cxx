                                                              ...
                                                              ServiceRegistryRef services, bool createNew)
{
  ...
  for (size_t ti = 0; ti < mTimesliceIndex.size(); ++ti) {
    ...
    for (size_t ei = 0; ei < expirationHandlers.size(); ++ei) {
      ...
      auto messageGetter = [&partial](size_t idx, DataRefIndices current) -> fair::mq::Message* {
      ...
      InputSpan span{nPartsGetter, refCountGetter, indicesGetter, nextIndicesGetter, messageGetter, static_cast<size_t>(partial.size())};
