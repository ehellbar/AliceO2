...
bool DataProcessingDevice::tryDispatchComputation(ServiceRegistryRef ref, std::vector<DataRelayer::RecordAction>& completed)
{
  ...
  auto getInputSpan = [ref, &currentSetOfInputs](TimesliceSlot slot, bool consume = true) {
    ...
    auto messageGetter = [&currentSetOfInputs](size_t i, DataRefIndices current) -> fair::mq::Message* {
      auto const& msgs = currentSetOfInputs[i];
    ...
    return InputSpan{nofPartsGetter, refCountGetter, indicesGetter, nextIndicesGetter, messageGetter, currentSetOfInputs.size()};
  };
