...
struct CCDBHelpers {
  ...
  /// Adopt a freshly-fetched CCDB payload as a new SHM message and
  /// atomically replace this path's entry in the producer's CacheId map.
  /// The new SHM message is adopted BEFORE the old cached one is pruned.
  /// @a allocator producer-device DPL DataAllocator
  /// @a cache the producer-local path -> CacheId map. Updated in place.
  /// @a path CCDB path
  ...
  /// @return the new CacheId; equal to cache[@a path] on return
  static DataAllocator::CacheId adoptAndReplaceCachedMessage(
    DataAllocator& allocator,
    std::unordered_map<std::string, DataAllocator::CacheId>& cache,
