...
struct CCDBHelpers {
  ...
  /// adopted BEFORE the old cached one is pruned. The caller is
  /// responsible for storing the returned CacheId in its path -> CacheId
  /// map (typically: cache[path] = cacheId).
